...
- [ ] В `backend/api/tasks.py` реализована и экспортирована функция `send_sms_verification_code_task`.
- [ ] Сервис `qcluster` перезапущен и успешно обрабатывает задачи отправки OTP.
- [ ] `docker compose exec backend python manage.py check` — 0 ошибок.
- [ ] `docker compose exec backend python manage.py makemigrations --check` — `No changes detected`.
- [ ] `npm run build` во `frontend/` — без ошибок сборки и компиляции TypeScript.
- [ ] Playwright E2E тест `mazory-otp-waha.spec.ts` проходит успешно: отправка инициируется через интерфейс, сообщение появляется в WAHA.
- [ ] Логи `docker compose logs --tail=50 backend qcluster waha` без ошибок.
- [ ] Изменения зафиксированы в ветке `bugfix/whatsapp-otp-send-task` и оформлен Pull Request в `main`.
